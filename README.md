...
3. Выполните команду `./vendor/bin/sail up -d`. Перед выполнением данной команды необходимо убедиться, что установлен docker compose и не заняты следующие порты: `80`, `6379`, `7700`, `3306`, `1025`
4. Далее необходимо зайти внутрь контейнера образа `sail-8.2/app` c помощью команды: `docker exec -it {CONTAINER ID | NAME} bash`. Чтобы посмотреть список контейнеров можно воспользоваться командой `docker ps`
5. Внутри контейнера необходимо запустить следующие команды:
   - `php artisan migrate`
6. Всё необходимое для проверки тестового задания готово. Можете ознакомиться с документацией API по ссылке [`http://localhost`](http://localhost)
